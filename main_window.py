...
from iterator import PigsIterator  # Импортируем ваш итератор
...
class ImageViewer(QMainWindow):
    ...
    def load_dataset(self) -> None:
        ...
        if file_path:
            try:
                self.dataset_iterator = iter(PigsIterator(file_path))  # Используем ваш итератор
                self.next_image_button.setEnabled(True)
                self.show_next_image()  # Отображаем первое изображение
    ...
    def show_next_image(self) -> None:
        ...
        if self.dataset_iterator:
            try:
                self.current_image_path = next(self.dataset_iterator)  # Получаем следующий путь
